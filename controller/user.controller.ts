...
export const loginUser = CatchAsyncFunction( async (req: Request, res: Response, next: NextFunction) => {
  const {email, password}: LoginInterface = req.body.data 
  ...
  //auth user
  sendToken(res, 200, isUserExist, true);
})
...
export const updateUserAccessToken = CatchAsyncFunction( async (req: Request, res: Response, next: NextFunction) => {
    //get refresh token
    // console.log('Refresh Token-> user constroller', req.cookies)
    // const refreshToken = req.cookies.refresh_token;
    // console.log('Refresh Token-> user constroller', refreshToken)
    console.log('updateuserAccessToken Function: ',req.headers)
    const refreshToken = ''

    //check for the user with the refresh token in db
...
//-------------Send User Request Email To Admin----------------
export const sendUserRequestToAdmin = CatchAsyncFunction( async(req: Request, res: Response, next:NextFunction) => {
  try {
    const {firstname, lastname, email, desc} = req.body.data
    console.log('fun sendUserRequestToAdmin', firstname)
    
    if(!firstname || !lastname || !email || !desc){
      return next(new ErrorHandler('All field must be valid', 400))
    }
   ...
   // send email
    sendMail({
      ...
      template: "user_inquries.ejs",
      data: { user: { firstname, lastname, email, desc }},
    });
